...
import { Plus, Home, Eye, MessageSquare, Settings, LogOut, Edit, Trash2 } from "lucide-react";
  ...
  const fetchSellerData = async () => {
    try {
      ...
      if (propertiesData.success) {
        ...
        const activeProperties = propertiesData.data.filter((p: Property) => p.status === 'active').length;
        const pendingProperties = propertiesData.data.filter((p: Property) => p.approvalStatus === 'pending').length;
        const totalViews = propertiesData.data.reduce((sum: number, p: Property) => sum + (p.views || 0), 0);
        const totalInquiries = propertiesData.data.reduce((sum: number, p: Property) => sum + (p.inquiries || 0), 0);
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Badge className={getApprovalStatusColor(property.approvalStatus)}>
