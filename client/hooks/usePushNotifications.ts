    ...
    try {
      ...
      wsRef.current.onclose = (event) => {
        ...
        if (event.code === 1000 || event.code === 1001 || event.code === 1006) {
          console.log(
            "📝 WebSocket closed, checking if server is available before reconnecting",
          );
          ...
          if (window.location.hostname.includes(".fly.dev")) {
            ...
            if (reconnectAttempts.current < 2) {
              ...
              console.log(
                `🔄 Production environment: Will retry WebSocket in ${delay}ms`,
              );
              setTimeout(() => {
              ...
              console.log(
                "⚠️ WebSocket service unavailable in production, notifications disabled",
              );
              setIsConnected(false);
            }
          }
        ...
        if (
          ...
          console.log(
            `🔄 Reconnecting to push notifications in ${delay}ms (attempt ${reconnectAttempts.current + 1}/${maxReconnectAttempts})`,
          );
          ...
          console.warn(
            "⚠️ Max reconnection attempts reached for push notifications",
          );
        }
      };
      ...
      wsRef.current.onerror = (error) => {
        ...
        try {
          ...
          if (!isProduction) {
            console.error(
              "Raw error object:",
              JSON.stringify(rawErrorInfo, null, 2),
            );
        ...
        if (
          ...
          console.log(
            `🔄 Attempting to reconnect push notifications in ${delay}ms...`,
          );
          setTimeout(() => {
            reconnectAttempts.current++;
            connectWebSocket();
          ...
          console.warn(
            "⚠️ Max reconnection attempts reached, user not authenticated, or WebSocket not closed properly",
          );
        }
      };
    } catch (error) {
      ...
      if (error instanceof Error && error.message.includes("WebSocket")) {
        console.warn(
          "⚠️ WebSocket connection failed, disabling notifications for this session",
        );
        reconnectAttempts.current = maxReconnectAttempts;
      }
    }
