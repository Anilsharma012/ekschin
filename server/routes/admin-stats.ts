...
router.get("/", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const propertyStats = await propertiesCollection
      .aggregate([
        {
          $group: {
            _id: "$status",
    ...
    const userGrowth =
      totalUsers > 0 ? ((lastMonthUsers / totalUsers) * 100).toFixed(1) : "0";
    const propertyGrowth =
      totalProperties > 0
        ? ((lastMonthProperties / totalProperties) * 100).toFixed(1)
        : "0";
    ...
    const stats = {
      ...
      insights: {
        ...
        averageRevenuePerProperty:
          totalProperties > 0 ? Math.round(revenue / totalProperties) : 0,
        mostActiveUserType: usersByType.reduce(
    ...
    if (
      ...
      console.log(
        "📊 Using fallback stats due to database connectivity issues",
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const userTypeDistribution = await usersCollection
        .aggregate([
          {
            $group: {
              _id: "$userType",
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const statusDistribution = await propertiesCollection
        .aggregate([
          {
            $group: {
              _id: "$status",
      ...
      // Properties by city
      const cityDistribution = await propertiesCollection
        .aggregate([
          {
            $group: {
              _id: "$location.city",
      ...
      const priceRanges = await propertiesCollection
        .aggregate([
          {
            $bucket: {
              groupBy: "$price",
